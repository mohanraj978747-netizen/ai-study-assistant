...
export default function Chat() {
  ...
  // Load saved conversations once on mount, so a refresh never starts a
  // blank new chat while history exists on the backend.
  useEffect(() => {
    let cancelled = false;
    chatService
      .getConversations()
      ...
      .finally(() => !cancelled && setLoadingConvos(false));
    return () => {
      cancelled = true;
    };
  }, []);
  ...
  const handleSend = async (e) => {
    ...
    try {
      if (!conversationId) {
        const convo = await chatService.createConversation(text.slice(0, 40));
      ...
      setError('Your tutor could not respond just now. Please try again.');
    } finally {
      setSending(false);
    }
  };
    ...
    <PageTransition className="flex h-full flex-col lg:flex-row">
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
                <span className="text-sm font-medium text-slate-200">Your chats</span>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center gap-3 border-b border-white/10 px-4 py-3 lg:px-6">
          ...
          <div>
            <h1 className="font-display text-base font-semibold text-parchment">Tutor chat</h1>
            <p className="text-xs text-slate-500">Ask about anything, not just your notes</p>
          </div>
        </div>

        <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto px-4 py-6 lg:px-6">
          {loadingMessages ? (
            <div className="flex h-full items-center justify-center">
              <Loader size="sm" />
            </div>
        ...
        {error && <p className="px-4 pb-2 text-xs text-rose-400 lg:px-6">{error}</p>}

        <form onSubmit={handleSend} className="border-t border-white/10 p-3 lg:p-4">
